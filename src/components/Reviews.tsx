...
const Reviews = () => {
  ...
  return (
    <section id="reviews" className={`px-[6vw] py-28 relative overflow-hidden transition-colors duration-300 ${dark ? 'bg-[#05091a]' : 'bg-white'}`}>
      <div className="absolute inset-0 pointer-events-none" style={{
        backgroundImage: `linear-gradient(rgba(26,107,255,${dark ? '0.03' : '0.02'}) 1px, transparent 1px), linear-gradient(90deg, rgba(26,107,255,${dark ? '0.03' : '0.02'}) 1px, transparent 1px)`,
      ...
      <div className="text-center max-w-lg mx-auto mb-16 relative">
        ...
        <h2 style={{ fontFamily: 'var(--font-display)' }}
          className={`text-5xl font-extrabold leading-none uppercase transition-colors duration-300 ${dark ? 'text-white' : 'text-[#05091a]'}`}>
          What Our Clients<br />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative">
        {reviews.map(({ initials, name, role, text }) => (
          <div key={name}
            className={`border p-8 hover:-translate-y-1 transition-all group ${
              dark
                ? 'bg-[#0d1635] border-[#1a6bff]/20 hover:border-[#1a6bff]/50 hover:shadow-2xl hover:shadow-[#1a6bff]/10'
                : 'bg-[#f5f8ff] border-blue-100 hover:border-[#1a6bff]/30 hover:shadow-xl hover:shadow-blue-100'
            }`}
          >
            ...
            <p className={`leading-relaxed text-sm mb-8 transition-colors duration-300 ${dark ? 'text-gray-300 font-light' : 'text-gray-600'}`}>{text}</p>
            <div className={`flex items-center gap-3 pt-6 border-t transition-colors duration-300 ${dark ? 'border-[#1a6bff]/10' : 'border-blue-100'}`}>
              <div
                className="w-10 h-10 bg-[#1a6bff] text-white flex items-center justify-center font-bold text-xs flex-shrink-0"
                style={{ fontFamily: 'var(--font-display)', clipPath: 'polygon(4px 0%, 100% 0%, calc(100% - 4px) 100%, 0% 100%)' }}
              >
                {initials}
              </div>
              <div>
                <div style={{ fontFamily: 'var(--font-display)', letterSpacing: '0.05em' }}
                  className={`font-semibold text-sm transition-colors duration-300 ${dark ? 'text-white' : 'text-[#05091a]'}`}
                >{name}</div>
